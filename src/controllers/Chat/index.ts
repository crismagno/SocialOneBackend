...
class ChatController {
  ...
  public setIdUserOnSeenMessages = async (
    ...
    try {
      ...
      let messagesIds: IMessageSchema[] = await Message.find(
        {
          userSent: { $ne: userId },
          chat: chatId,
      ...
      let messagesIdsTransform: string[] = messagesIds?.map(
        (message: IMessageSchema) => message?._id
      );
      ...
      const updateMessagesIds = await Message.updateMany(
        {
      ...
      console.log(
        `Error to setIdUserOnSeenMessages:
        [userId: ${userId}]
        [chatId: ${chatId}]`,
        error
      );
  ...
  public setIdUserOnSeenMessageSingle = async (
    ...
    try {
      ...
      console.log(
        `Error to setIdUserOnSeenMessageSingle:
        [userId: ${data.userId}]
        [chatId: ${data.chatId}]`,
        error
      );
  ...
  public create = async (
    ...
    try {
      ...
      const chatWithThisCreatorAndPersonExist: IChatSchema | null =
        await Chat.findOne({
          $or: [{ users: [creator, person] }, { users: [person, creator] }],
        }).populate(this.populateChatDefault);

      // verify if chat with only two users already exist
      if (chatWithThisCreatorAndPersonExist) {
        return res.status(201).json({
          message: "Error chat with user already exist",
          chat: chatWithThisCreatorAndPersonExist,
        });
      }
      ...
      const messageCreated: IMessageSchema = await Message.create(
        messageCreate
      );

      // update last message on chat
      const chatUpdateMessageStart: IChatSchema | null =
  ...
  public getChatsByUser = async (
    ...
    try {
      ...
      if (searchValue?.trim()) {
        usersBySearchValue = await User.distinct("_id", {
        ...
        if (usersBySearchValue == null) {
          return res.status(400).json({
            message: "Error to find users by search value, not found",
          });
        }
        ...
        if (idsUsersSearchValue.length > 0) {
          queryChats = {
            $and: [{ users: userId }, { users: { $in: idsUsersSearchValue } }],
          };
        } else {
          queryChats = {
            users: [],
      ...
      const chats: IChatSchema[] | null = await Chat.find(queryChats)
        .skip(parseInt(skip))
      ...
      if (chats == null) {
        return res
          .status(400)
          .json({ message: "Error to find chats not found" });
      }

      return res
        .status(200)
        .json({ message: "Chats find with success!", chats });
    } catch (error) {
      console.log(
        `Error to getChatsByUser: [userId: ${req.body.userId}]`,
        error
      );
  ...
  public getChatById = async (
    ...
    try {
      ...
      const chat: IChatSchema | null = await Chat.findById(chatId).populate(
        this.populateChatDefault
      );

      if (!chat) {
        return res
          .status(400)
          .json({ message: "Error to find chat not found" });
      }
